data segment
    ...
    msg2 db ": $"
    msg3 db 0dh, 0ah, 0dh, 0ah, "grand total: $"
    msg4 db 0dh, 0ah, "$"
...
main:
    add al, [si]
    daa             ; daa (decimal adjust addition) uses "al" register
    jnc skip

    mov bl, al
    mov al, ah
    add al, 01h     ; since daa works based on lower nibble > 9 or when the auxillary carry is set
                    ; hence we need to perform an arithmetic operation so that the flags are set
    daa
    mov ah, al
    mov al, bl
